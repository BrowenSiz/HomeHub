...
const dragTargetName = computed(() => {
  if (currentView.value === 'album_detail' && activeAlbum.value) {
    return `альбом «${activeAlbum.value.name}»`
  }
  if (currentView.value === 'vault' && authStore.isVaultUnlocked) {
    return 'личный сейф'
  }
  return 'библиотеку'
})
...
<template>
  <div 
    class="h-screen w-full flex p-5 gap-5 overflow-hidden relative selection:bg-blue-500 selection:text-white font-sans bg-[#0f172a]"
